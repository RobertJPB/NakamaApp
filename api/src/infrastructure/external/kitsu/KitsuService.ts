...
import { crearCacheConRespaldoRedis } from '../../services/RedisCache'

const KITSU_URL = 'https://kitsu.io/api/edge'

// Cache para notas MAL: kitsuId -> score
const MAL_CACHE_TTL_MS = 24 * 60 * 60 * 1000 // 24 horas
const malScoreCache = crearCacheConRespaldoRedis<string, { score: number }>(1000, MAL_CACHE_TTL_MS)

// Cache general para respuestas de API (búsquedas y populares)
const RESPONSE_CACHE_TTL = 60 * 60 * 1000 // 1 hora
const responseCache = crearCacheConRespaldoRedis<string, any>(500, RESPONSE_CACHE_TTL)
...
export async function translateText(text: string): Promise<string | null> {
  ...
  try {
    const res = await fetch(
      `https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=es&dt=t`,
      {
        method: 'POST',
  ...
  try {
    const res = await fetch(
      `https://clients5.google.com/translate_a/t?client=dict-chrome-ex&sl=auto&tl=es&q=${encodeURIComponent(text.substring(0, 1500))}`
    )
    if (res.ok) {
      const data = (await res.json()) as any
      const clean = data[0][0]
        .replace(/\r\n/g, '\n')
        .replace(/([^\n])\n([^\n])/g, '$1 $2')
        .trim()
      if (validar(clean)) return clean
...
export class KitsuService implements IAnimeExternalService {
  ...
  private async fetchKitsuPaginated(
    baseEndpoint: string,
    limitRequerido: number,
    ...
    const combinedIncluded = results.reduce(
      (acc: any[], curr: any) => acc.concat(curr.included || []),
      []
    )
  ...
  async obtenerDetalle(externalId: string) {
    ...
    const data = await this.fetchKitsu(
      `/anime/${externalId}?include=categories,characters,characters.character`
    )
    ...
    if (unknownGenres.length > 0) {
      ...
      if (translatedStr) {
        ...
        unknownGenres.forEach((cat, index) => {
          generos.push(
            translatedArr[index] ? translatedArr[index].replace(/(^[\s,]+|[\s,]+$)/g, '') : cat
          )
        })
      } else {
    ...
    const personajes = included
      ...
      .map((char: any) => ({
        id: char.id,
        nombre:
          char.attributes?.names?.en ||
          char.attributes?.canonicalName ||
    ...
    if (personajes.length === 0) {
      try {
        ...
        const mappingRes = await fetch(`${KITSU_URL}/anime/${externalId}/mappings`, {
          headers: { Accept: 'application/vnd.api+json' },
        })
        if (mappingRes.ok) {
          const mappingData = (await mappingRes.json()) as any
    ...
    if (animeMapping.titulo?.toLowerCase().includes('orb: on the movements')) {
      animeMapping.sinopsis =
        'En la Europa del siglo XV, la herejía es castigada con la quema en la hoguera. Rafal, un niño prodigio, está destinado a especializarse en teología en la universidad, pero su encuentro con un misterioso erudito lo lleva a descubrir una verdad prohibida: que la Tierra se mueve alrededor del Sol.\n\nAtrapado entre su sed de conocimiento y la implacable persecución de la Inquisición, Rafal deberá decidir si vale la pena arriesgar su propia vida por proteger y transmitir esta revolucionaria investigación a las futuras generaciones.'
      ...
      personajes.push(
        {
          ...
          nombre: 'Nowak',
          imagenUrl: wrapProxy(
            'https://s4.anilist.co/file/anilistcdn/character/large/b279996-MdHZp8RFDafh.png'
          ),
        },
        {
          id: '280003',
          nombre: 'Draka',
          imagenUrl: wrapProxy(
            'https://s4.anilist.co/file/anilistcdn/character/large/b280003-Fu17utO1VAsD.png'
          ),
        },
        {
          id: '280005',
          nombre: 'Schmitt',
          imagenUrl: wrapProxy(
            'https://s4.anilist.co/file/anilistcdn/character/large/b280005-7OvMzRzuFVPx.png'
          ),
        },
        {
          id: '280002',
          nombre: 'Oczy',
          imagenUrl: wrapProxy(
            'https://s4.anilist.co/file/anilistcdn/character/large/b280002-3E9GkKZ8YH9D.png'
          ),
        },
        {
          id: '279999',
          nombre: 'Badeni',
          imagenUrl: wrapProxy(
            'https://s4.anilist.co/file/anilistcdn/character/large/b279999-VKadGXZplQiE.png'
          ),
        },
        {
          id: '280001',
          nombre: 'Rafał',
          imagenUrl: wrapProxy(
            'https://s4.anilist.co/file/anilistcdn/character/large/b280001-40iXrV4vLyqR.png'
          ),
        },
        {
          id: '279998',
          nombre: 'Jolenta',
          imagenUrl: wrapProxy(
            'https://s4.anilist.co/file/anilistcdn/character/large/b279998-Jxzcihblmclf.png'
          ),
        },
        {
          id: '279995',
          nombre: 'Kolbe',
          imagenUrl: wrapProxy(
            'https://s4.anilist.co/file/anilistcdn/character/large/b279995-4n4fhISlYg9Z.png'
          ),
        }
      )
    }
  ...
  async obtenerPopulares(
    ...
    if (hayFiltros) {
      ...
      const pool = await this.obtenerPoolPorGenero({
        genre,
        seasonYear,
        tipo,
        demografia,
  ...
  private async obtenerPoolPorGenero(params: {
    genre?: string
    seasonYear?: number
    tipo?: string
    demografia?: string
  ...
  async buscarPersonajes(busqueda: string) {
    try {
      ...
      const [charsJson, animeJson] = (await Promise.all([
        fetch(
          `https://kitsu.io/api/edge/characters?filter[name]=${encodeURIComponent(busqueda)}&page[limit]=15&fields[character]=canonicalName,image`
        )
          .then((r) => r.json())
          .catch(() => null),
        fetch(
          `https://kitsu.io/api/edge/anime?filter[text]=${encodeURIComponent(busqueda)}&page[limit]=1&fields[anime]=id`
        )
          .then((r) => r.json())
          .catch(() => null),
      ])) as [any, any]
      ...
      if (topAnimeId) {
        try {
          ...
          animeChars = (animeCharsJson?.included || []).filter(
            (inc: any) => inc.type === 'characters' && inc.attributes?.image
          )
        } catch (err) {
