...
export function mapKitsuToAnime(kitsuData: any): Partial<Anime> {
  ...
  return {
    ...
    titulo: attrs.titles?.en || attrs.titles?.en_jp || attrs.titles?.en_us || attrs.canonicalTitle || 'Sin título',
    ...
    estadoEmision: attrs.status === 'finished' ? 'Terminado' : 
                   attrs.status === 'current' ? 'En Emisión' : 
                   attrs.status === 'tba' ? 'TBA' : 
                   (attrs.status === 'unreleased' || attrs.status === 'upcoming') ? 'Próximamente' : 
    ...
    calificacionPromedio: attrs.averageRating ? parseFloat((parseFloat(attrs.averageRating) / 10).toFixed(1)) : 0,
  }
}
