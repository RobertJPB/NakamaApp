...
import { crearCacheConRespaldoRedis } from '../../../infrastructure/services/RedisCache'

// Cache de resultados completos para evitar DB upsert + counts en cada petición
const RESULT_CACHE_TTL = 60 * 60 * 1000 // 1 hora
const resultCache = crearCacheConRespaldoRedis<string, any>(200, RESULT_CACHE_TTL)
...
export class ObtenerDetalleAnime {
  ...
  async execute(externalId: string) {
    ...
    const localSinopsisValida =
      animeLocal?.sinopsis && !/QUERY LENGTH/i.test(animeLocal.sinopsis)
        ? animeLocal.sinopsis
        : null
    ...
    const [resenas, viendo, porVer, favoritos] = await Promise.all([
      ...
      this.prisma.listaUsuario.count({
